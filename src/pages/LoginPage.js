...
const LoginPage = () => {
  ...
  const login = () => {
    if (userId === '' || password === '') {
      return window.alert('아이디 혹은 비밀번호가 공란입니다! 입력해주세요');
    }
    ...
    <>
      ...
      <Grid textAlign='center' width='60%' margin='auto'>
        ...
        >
          ...
          <hr />
          <Text>
            가입하기 로그인 Messenger Facebook LiteWatch 사람 페이지 페이지
            카테고리 장소 게임 위치 Marketplace Facebook Pay 그룹 채용 정보
            Oculus Portal Instagram 지역 기부 캠페인 서비스 투표 정보 센터 정보
            광고 만들기 페이지 만들기 개발자 채용 정보 개인정보처리방침 쿠키
            AdChoices 이용 약관 고객 센터 설정활동 로그
          </Text>
        </Grid>
      </Grid>
    </>
